...
// for Compute 3.0
// NOTE: compile this .cu module for compute_52,sm_30 with --maxrregcount=63
//
...
bool NVKernel::bindtexture_1D(uint32_t *d_V, size_t size)
{
	...
	checkCudaErrors(cudaBindTextureToArray(NULL, &texRef1D_4_V, d_V, &channelDesc4, size));
	return true;
}
...
bool NVKernel::bindtexture_2D(uint32_t *d_V, int width, int height, size_t pitch)
{
	...
	checkCudaErrors(cudaBindTextureToArray2D(NULL, &texRef2D_4_V, d_V, &channelDesc4, width, height, pitch));
	return true;
}
